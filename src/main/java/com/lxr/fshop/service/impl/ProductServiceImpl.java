...
import com.lxr.fshop.mapper.ProductMapper;
...
import com.lxr.fshop.service.ReviewService;
@Service
public class ProductServiceImpl implements ProductService {
    ...
    public List list(int cid) {
        ...
        example.createCriteria().andCidEqualTo(cid).andStatusNotEqualTo(0);
    ...
    @Override
    public List listAdmin(int cid) {
        ProductExample example = new ProductExample();
        example.createCriteria().andCidEqualTo(cid).andStatusNotEqualTo(0);//0代表下架
        example.setOrderByClause("id desc");
        List result = productMapper.selectByExample(example);
        setFirstProductImage(result);
        setCategory(result);
    ...
    public List<Product> search(String keyword) {
        ProductExample example = new ProductExample();
        example.createCriteria().andNameLike("%" + keyword + "%").andStatusNotEqualTo(0);
        example.setOrderByClause("id desc");
        List result = productMapper.selectByExample(example);
        setFirstProductImage(result);
        setCategory(result);
